...
async def test_create(settings, conf_path):
    conf = {'settings': settings,
            'devices': []}

    srv = await hat.manager.server.create_server(conf, conf_path)
...
async def test_connect(settings, conf_path, addr, conn_count):
    conf = {'settings': settings,
            'devices': []}
    srv = await hat.manager.server.create_server(conf, conf_path)
...
async def test_save(settings, conf_path, addr):
    conf = {'settings': settings,
            'devices': []}
    srv = await hat.manager.server.create_server(conf, conf_path)
...
async def test_set_settings(settings, conf_path, addr,
                            patch_autoflush):
    conf = {'settings': settings,
            'devices': []}
    srv = await hat.manager.server.create_server(conf, conf_path)
...
async def test_init_device(settings, conf_path, addr,
...
async def test_add_remove_device(settings, conf_path, addr,
...
async def test_start_stop_device(settings, conf_path, addr,
                                 patch_autoflush, patch_device_queue):
    conf = {'settings': settings,
            'devices': []}
    srv = await hat.manager.server.create_server(conf, conf_path)
...
async def test_execute(settings, conf_path, addr,
                       patch_autoflush, patch_device_queue):
    conf = {'settings': settings,
            'devices': []}
    srv = await hat.manager.server.create_server(conf, conf_path)
...
async def test_autostart(settings, conf_path, addr,
                         patch_autoflush, patch_device_queue,
                         patch_autostart):
    conf = {'settings': settings,
            'devices': []}
